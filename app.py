...
def get_next_videos_from_library(page_id, count=1):
    """جلب الفيديو التالي من المكتبة بالترتيب"""
    try:
        url = f"{VIDEO_LIBRARY_URL}/api/publisher/next"
        r = requests.get(url, params={'count': count, 'page_id': page_id}, timeout=30)
        data = r.json()
...
def download_and_post_video(page_id, access_token, video_url, caption):
    """تحميل الفيديو مؤقتاً ورفعه على فيسبوك ثم حذفه"""
...
def post_video_from_library(page_id, access_token, fb_page_id, topic, personality):
    """جلب فيديو من المكتبة ونشره مع تعليق AI"""
    videos = get_next_videos_from_library(page_id, count=1)
    if not videos:
        logger.warning(f"No videos in library for page {page_id}")
        return False, 'لا توجد فيديوهات في المكتبة'
    video = videos[0]
    video_url = video.get('nowm_url') or video.get('play_url', '')
    title = video.get('title', '')
    author = video.get('author', '')
    if not video_url:
        return False, 'رابط الفيديو غير موجود'
    # توليد تعليق بالـ AI
    ...
    if api_key:
        try:
            ...
            prompt = (f'اكتب تعليقاً إسلامياً مناسباً لفيديو عنوانه: "{title}" '
                     f'لصفحة موضوعها: {topic}\n'
                     f'الأسلوب: {p_name}\n'
                     f'التعليق يكون قصيراً ومؤثراً 2-3 أسطر + هاشتاقات إسلامية مناسبة\n'
                     f'باللغة العربية فقط')
            ...
            if 'choices' in data:
                ...
                if has_foreign_words(caption):
                    caption = clean_foreign_words(caption)
            else:
        ...
        caption = f"🎬 {title}\n\n#إسلاميات #محتوى_إسلامي"
    return download_and_post_video(fb_page_id, access_token, video_url, caption)
...
def video_library_status():
    try:
        r = requests.get(f"{VIDEO_LIBRARY_URL}/api/stats", timeout=10)
        data = r.json()
        pub_r = requests.get(f"{VIDEO_LIBRARY_URL}/api/publisher/stats", timeout=10)
        pub_data = pub_r.json()
        return jsonify({'online': True, 'stats': data, 'publisher': pub_data})
    except Exception as e:
        return jsonify({'online': False, 'error': str(e)})

@app.route('/api/video_library/test', methods=['POST'])
def video_library_test():
    data = request.json or {}
    page_id = data.get('page_id', 'test')
    try:
        r = requests.get(f"{VIDEO_LIBRARY_URL}/api/publisher/next",
            params={'count': 1, 'page_id': page_id}, timeout=15)
        result = r.json()
