...
from db import db_get, db_set, init_db, db_mode
try:
...
LOGS_KEY       = 'logs'
STATS_KEY      = 'stats'
QURAN_KEY      = 'quran_progress'
CALENDAR_KEY   = 'content_calendar'
WEEKLY_KEY     = 'weekly_stats'
ANALYTICS_KEY  = 'analytics'

# للتوافق مع الكود القديم
SETTINGS_FILE  = SETTINGS_KEY
PAGES_FILE     = PAGES_KEY
LOGS_FILE      = LOGS_KEY
STATS_FILE     = STATS_KEY
QURAN_FILE     = QURAN_KEY
CALENDAR_FILE  = CALENDAR_KEY
WEEKLY_FILE    = WEEKLY_KEY
...
VIDEO_PROGRESS_FILE = 'video_progress'
VIDEO_DAILY_FILE    = 'video_daily'
DROPBOX_SPACES_FILE = 'dropbox_spaces'
...
def get_next_video_from_dropbox(page_id):
...
def post_video_from_library(page_id, access_token, fb_page_id, topic, personality):
    """جلب فيديو من Dropbox ونشره على فيسبوك"""
    if not DROPBOX_REFRESH_TOKEN:
        return False, 'Dropbox Token غير مضبوط'
    video = get_next_video_from_dropbox(page_id)
    if not video:
    ...
    if api_key:
        try:
            ...
            system_msg = 'أنت كاتب محتوى إسلامي متخصص. اكتب باللغة العربية الفصحى فقط بدون أي حروف أجنبية أو إنجليزية.'
            prompt = (
                'اكتب تعليقاً إسلامياً مؤثراً لفيديو إسلامي.\n'
                'اسم الفيديو: ' + clean_name + '\n'
                'موضوع الصفحة: ' + topic + '\n\n'
                'التعليق يجب أن:\n'
                '- يبدأ بآية قرآنية أو حديث نبوي مناسب\n'
                '- يدعو للتفكر في رحمة الله وحبه لعباده\n'
                '- يشجع على قراءة القرآن الكريم\n'
                '- يكون 3-4 أسطر فقط\n'
                '- يختم بهاشتاقات إسلامية عربية\n'
...
def publish_scheduled_post(page_id_str, schedule_id):
    ...
    if post_type == '🎬 فيديو من المكتبة':
        success, result = post_video_from_library(
            ...
            page.get('topic','عام'), personality
        )
        mode = '🎬'
...
def setup_scheduler():
    """
    نظام هجين:
    - APScheduler للبيئات التي تدعمه (Railway)
    - cron-job.org endpoint كـ fallback موثوق
    """
    try:
        ...
        for page in pages:
            ...
            for schedule in page.get('schedules', []):
                try:
                    ...
                    scheduler.add_job(publish_scheduled_post,
        ...
        logger.info(f"Scheduler: {len(scheduler.get_jobs())} jobs loaded")
    except Exception as e:
        logger.error(f"setup_scheduler error: {e}")

def check_and_publish_due():
    """
    الدالة الرئيسية للنشر التلقائي - تُستدعى من cron-job.org كل دقيقة
    تتحقق من كل المواعيد وتنشر اللي حان وقتها
    """
    now = datetime.now(pytz.timezone('Asia/Riyadh'))
    ...
    for page in pages:
        ...
        for schedule in page.get('schedules', []):
            try:
                sched_time = schedule.get('time', '00:00')
                h, m = map(int, sched_time.split(':'))
                # تحقق من الوقت (± دقيقة واحدة)
                if h != current_hour or abs(m - current_minute) > 1:
                    continue
                # تحقق من اليوم
                days = schedule.get('days', [])
                if days and current_weekday not in days:
                    continue
                # تحقق أنه لم يُنشر في آخر 30 دقيقة
                last_key = f"last_pub_{page['id']}_{schedule['id']}"
                last = load_json(last_key, 0)
                if isinstance(last, (int, float)) and (now.timestamp() - last) < 1800:
                    continue
                # نشر
                logger.info(f"Auto-publishing: {page['name']} at {sched_time}")
                publish_scheduled_post(page['id'], schedule['id'])
                save_json(last_key, now.timestamp())
                published.append({'page': page['name'], 'time': sched_time})
            except Exception as e:
                logger.error(f"check_and_publish error: {e}")
...
def cron_trigger():
    """
    Endpoint لـ cron-job.org — يُستدعى كل دقيقة
    يتحقق من المواعيد وينشر تلقائياً
    """
    try:
        published = check_and_publish_due()
        logger.info(f"Cron run: {len(published)} posts published")
        return jsonify({
            'ok': True,
            'time': datetime.now(pytz.timezone('Asia/Riyadh')).strftime('%H:%M'),
            'published': published,
            'count': len(published)
        })
    except Exception as e:
...
try:
    init_db()
    logger.info(f"DB mode: {db_mode()}")
except Exception as e:
    logger.error(f"DB init failed: {e}")

# تشغيل APScheduler كـ backup
try:
    if not scheduler.running:
        scheduler.start()
    setup_scheduler()
    logger.info("APScheduler started as backup")
except Exception as e:
    logger.warning(f"APScheduler failed (OK - using cron endpoint): {e}")
