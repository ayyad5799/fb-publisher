...
def ping():
    """Keepalive endpoint لمنع نوم Render"""
    if not scheduler.running:
        scheduler.start()
        setup_scheduler()
        logger.info("Scheduler restarted via /ping")
    return jsonify({'status': 'alive', 'scheduler': scheduler.running, 'time': datetime.now(pytz.timezone('Asia/Riyadh')).strftime('%Y-%m-%d %H:%M:%S')})
