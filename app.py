...
YOUTUBE_API_KEY = 'AIzaSyCkdvhUgb2wMfXdUULCEHgYxAbP1iSdugI'
...
POST_TYPES = {
    ...
    'youtube_video': 'فيديو يوتيوب تلقائي'
}
...
YOUTUBE_QUERIES = {
    'quran':         'تلاوة قرآن كريم قصيرة',
    'quran_tafseer': 'تفسير آية قرآنية',
    'hadith':        'حديث نبوي شريف',
    'duaa':          'دعاء مؤثر',
    'friday':        'فضل يوم الجمعة',
    'prophet_story': 'قصة نبي',
    'sahabi_story':  'قصة صحابي',
    'islamic_fact':  'معلومة إسلامية',
    'motivation':    'تحفيز إسلامي',
    'wisdom':        'حكمة إسلامية',
    'default':       'قرآن كريم'
}
...
def get_smart_post_type(page_id, requested_type):
    if requested_type in ['quran_tafseer','duaa','friday','seasonal','youtube_video']: return requested_type
    calendar = load_json(CALENDAR_FILE, {})
    history = calendar.get(page_id, [])
    last_3 = history[-3:]
    if requested_type in last_3:
        all_types = [t for t in POST_TYPES.keys() if t not in ['quran_tafseer','duaa','friday','seasonal','youtube_video']]
...
# ====== YouTube Functions ======
def search_youtube_video(post_type):
    """البحث عن فيديو يوتيوب عربي مناسب"""
    try:
        query = YOUTUBE_QUERIES.get(post_type, YOUTUBE_QUERIES['default'])
        ...
        if 'items' in data and data['items']:
            # فلتر: عنوان عربي فقط
            arabic_videos = [v for v in data['items']
                if any('؀' <= c <= 'ۿ' for c in v['snippet']['title'])]
            if arabic_videos:
                video = random.choice(arabic_videos)
            else:
...
def post_youtube_video_to_facebook(page_id, access_token, post_type, caption_text):
    """تحميل فيديو يوتيوب مؤقتاً ورفعه على فيسبوك ثم حذفه فوراً"""
    tmp_path = None
    try:
        video_info = search_youtube_video(post_type)
        if not video_info:
            return False, 'لم يتم العثور على فيديو مناسب'
        logger.info(f"Downloading video: {video_info['title']}")
        import subprocess, tempfile
        tmp = tempfile.NamedTemporaryFile(suffix='.mp4', delete=False, dir='/tmp')
        tmp_path = tmp.name
        tmp.close()
        # تحميل الفيديو مؤقتاً (أقصى 5 دقائق)
        result = subprocess.run([
            'yt-dlp',
            '-f', 'best[ext=mp4][height<=480]/best[ext=mp4]/best/worst',
            '--max-filesize', '80m',
            ...
            f"https://www.youtube.com/watch?v={video_info['video_id']}"
        ], capture_output=True, text=True, timeout=360)
        logger.info(f"yt-dlp stdout: {result.stdout[:300]}")
        logger.info(f"yt-dlp stderr: {result.stderr[:300]}")
        # تحقق من وجود الملف بأي امتداد
        actual_path = tmp_path
        for ext in ['.mp4', '.webm', '.mkv', '']:
            candidate = tmp_path.replace('.mp4', ext) if ext else tmp_path
        ...
        fb_result = r.json()
        if 'id' in fb_result:
            logger.info(f"Video posted successfully: {fb_result['id']}")
            return True, fb_result['id']
        error_msg = fb_result.get('error', {}).get('message', 'خطأ في رفع الفيديو')
        return False, error_msg
    except subprocess.TimeoutExpired:
        return False, 'انتهت مهلة التحميل'
    except Exception as e:
        logger.error(f"YouTube video post error: {e}")
        return False, str(e)
    finally:
        # حذف الفيديو فوراً بعد الرفع أو الفشل
        for ext in ['.mp4', '.webm', '.mkv', '']:
            try:
                candidate = tmp_path.replace('.mp4', ext) if (tmp_path and ext) else tmp_path
                if candidate and os.path.exists(candidate):
                    os.remove(candidate)
                    logger.info(f"Temp video deleted: {candidate}")
            except: pass
...
باللغة العربية فقط.'''
    if post_type == 'duaa':
        return f'اكتب منشور دعاء الصباح\n🤲 دعاء مأثور + شرح + تشجيع\nالأسلوب: {pname}'
    if post_type == 'friday':
        return f'اكتب منشور يوم الجمعة المبارك\n🕌 فضل الجمعة + سننها + دعاء\nالأسلوب: {pname}'
    if post_type == 'seasonal' and seasonal_ctx:
        ctx = random.choice(seasonal_ctx)
        return f'اكتب منشور خاص بـ: {ctx}\nالموضوع: {topic}\nالأسلوب: {pname}'
    prompts = {
        'tips': f'اكتب نصيحة مفيدة عن: {topic}\nالأسلوب: {pname}',
        'offer': f'اكتب بوست تسويقي عن: {topic}\nالأسلوب: {pname}',
        'question': f'اكتب سؤالاً تفاعلياً عن: {topic}\nالأسلوب: {pname}',
        'fact': f'اكتب معلومة مثيرة عن: {topic}\nالأسلوب: {pname}',
        'motivation': f'اكتب منشوراً تحفيزياً عن: {topic}\nالأسلوب: {pname}',
        'quran': f'اختر آية قرآنية عن: {topic}\nالآية + تفسيرها + الدروس\nالأسلوب: {pname}',
        'prophet_story': f'اكتب قصة نبي مناسبة لـ: {topic}\nالنبي + الموقف + الدرس + آية\nالأسلوب: {pname}',
        'sahabi_story': f'اكتب قصة صحابي مناسبة لـ: {topic}\nالصحابي + الموقف + الدرس\nالأسلوب: {pname}',
        'islamic_fact': f'اكتب معلومة إسلامية عن: {topic}\nالأسلوب: {pname}',
        'hadith': f'اذكر حديثاً صحيحاً عن: {topic}\nالحديث + راويه + شرحه + التطبيق\nالأسلوب: {pname}',
        'wisdom': f'''اكتب منشور عن حكمة لأحد عظماء الأدب العربي (اختر عشوائياً من: نجيب محفوظ، جبران خليل جبران، المتنبي، أحمد شوقي، طه حسين، الشابي، نزار قباني)
💡 الحكمة كاملة مع اسم قائلها + شرحها + كيف نطبقها + سؤال تفاعلي
الموضوع: {topic}، الأسلوب: {pname}''',
        'poet_quote': f'''اكتب منشور عن بيت شعر من الأدب العربي (اختر عشوائياً من: المتنبي، نزار قباني، محمود درويش، الشابي، أحمد شوقي)
🎭 البيت الشعري مع اسم الشاعر + شرحه + رسالته
الموضوع: {topic}، الأسلوب: {pname}''',
        'video_idea': f'اكتب نص بوست يقدم فيديو عن: {topic}\nالأسلوب: {pname}'
    }
...
def generate_content(topic, post_type, page_name, page_id='', personality='mixed'):
    ...
    if post_type not in ['quran_tafseer','duaa','friday','seasonal','youtube_video']:
        if seasonal_ctx and random.random() < 0.25: post_type = 'seasonal'
        elif now_tz.hour in [5,6,7,8] and random.random() < 0.35: post_type = 'duaa'
        elif now_tz.weekday() == 4 and random.random() < 0.4: post_type = 'friday'
    if not api_key:
        return generate_fallback_content(topic, post_type), post_type
    try:
        angles = ['بأسلوب قصصي شيق','بطريقة مباشرة وعملية','بأسلوب يثير الفضول',
                  'بطريقة عاطفية ومؤثرة','بأسلوب الخبير المتخصص','بطريقة خفيفة وممتعة']
        ...
        system_prompt = """أنت كاتب محتوى عربي محترف.
قواعد صارمة:
1. اكتب باللغة العربية فقط - ممنوع أي حرف أجنبي
2. ممنوع أي حرف لاتيني أو روسي أو غير عربي
3. ابدأ مباشرة بالمحتوى
4. استخدم الإيموجي المناسبة
5. اختم بدعوة للتفاعل"""
        full_prompt = f"""اكتب منشور فيسبوك عربي مميز.
{prompt_text}
الأسلوب: {angle} | رقم: {seed}
الطول: ١٥٠-٣٠٠ كلمة
أضف في النهاية: {hashtags}"""
        ...
        if 'choices' in data and data['choices']:
            ...
            if has_foreign_words(content):
                logger.warning("Foreign words detected, retrying...")
                payload['messages'][0]['content'] += "\n\nتحذير: الرد السابق احتوى على كلمات أجنبية. عربية فقط."
                ...
                if 'choices' in d2 and d2['choices']:
                    ...
                    if has_foreign_words(content):
                        content = clean_foreign_words(content)
            if page_id: record_post_type(page_id, post_type)
...
def publish_scheduled_post(page_id_str, schedule_id):
    ...
    if post_type == 'youtube_video':
        content, actual_type = generate_content(page.get('topic','عام'), 'quran', page['name'], page['id'], personality)
        success, result = post_youtube_video_to_facebook(page['page_id'], page['access_token'], 'quran', content)
    else:
    ...
    mode = '🎬' if post_type == 'youtube_video' else ('🖼️' if use_image else '📝')
...
def publish_page(page_id):
    ...
    if post_type == 'youtube_video':
        content, actual_type = generate_content(page.get('topic','عام'), 'quran', page['name'], page_id, personality)
        success, result = post_youtube_video_to_facebook(page['page_id'], page['access_token'], 'quran', content)
    else:
