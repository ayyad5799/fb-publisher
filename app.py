hQx4KKcz32kooDxHkpQxB7PtVFCsGg9pAMvpbTy6NXjF9ueyU3u8YtZpQDkKV-tx4Y3fGUOdZsz-lYqJWJY8WVOvzsl2VqGhJkle2rcEbyC7-VDzfqr7AeH15dSZuF8jmTnz3HmD1ShEmYOi3jI8Q11jhe-vtIYv58WYiEhRg220DPVDHegoJq6WgenzUl2hE5z3Qg0h-W9AZUGJUf8FTJVPFuhECBbXrsxAg')
...
def delete_dropbox_video(file_path):
    """حذف الفيديو من Dropbox بعد النشر الناجح"""
    try:
        headers = {
        ...
        r = requests.post('https://api.dropboxapi.com/2/files/delete_v2',
            ...
            timeout=15)
        if r.status_code == 200:
            logger.info(f"Video deleted from Dropbox: {file_path}")
            return True
        else:
            logger.error(f"Dropbox delete error: {r.status_code} {r.text[:100]}")
            return False
    except Exception as e:
...
def get_next_video_from_dropbox(page_id):
    """يرجع الفيديو التالي للنشر بالترتيب"""
    try:
        videos = list_dropbox_videos()
        if not videos:
            return None
        ...
        if next_idx >= len(videos):
            next_idx = 0  # ابدأ من أول
        video = videos[next_idx]
        progress[page_id] = next_idx
...
def post_video_from_library(page_id, access_token, fb_page_id, topic, personality):
    ...
    caption = "🎬 " + file_name.rsplit(".",1)[0] + "\n\n#إسلاميات #محتوى_إسلامي"
    if api_key:
        try:
            p_name = PAGE_PERSONALITIES.get(personality, 'متنوع')
            clean_name = file_name.rsplit('.', 1)[0].replace('_', ' ').replace('-', ' ')
            prompt = ('اكتب تعليقاً إسلامياً مناسباً لفيديو اسمه: ' + clean_name
                     + '\nلصفحة موضوعها: ' + topic
                     + '\nالأسلوب: ' + p_name
                     + '\nالتعليق 2-3 أسطر + هاشتاقات إسلامية\nباللغة العربية فقط')
            ...
            if 'choices' in data:
                ...
                if cap and not has_foreign_words(cap):
                    caption = cap
                elif cap:
                    caption = clean_foreign_words(cap)
        except Exception as e:
    ...
    try:
        ...
        if 'id' in result:
            ...
            # حذف الفيديو من Dropbox بعد النشر
...
def video_library_status():
    if not DROPBOX_TOKEN:
        return jsonify({'online': False, 'configured': False, 'error': 'Dropbox Token غير مضبوط'})
    try:
        videos = list_dropbox_videos()
        progress = load_video_progress()
        return jsonify({
    ...
    try:
        ...
        return jsonify({'success': True, 'count': len(videos),
            'videos': [{'name': v['name'], 'size': v.get('size', 0),
                        'modified': v.get('server_modified','')} for v in videos[:20]]})
    except Exception as e:
