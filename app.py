...
# استخدم /data لو موجود (Render Disk) وإلا استخدم مجلد محلي
...
SCHEDULE_TEMPLATES = {
    ...
    "🔥 جدول الذروة الإسلامي": {
        "desc": "12 بوست على أوقات أعلى تفاعل للجمهور العربي + 5 فيديوهات",
        "hours": [
            (6,  "🤲 الأذكار والأدعية"),
            (8,  "📖 القرآن الكريم"),
            (10, "🎬 فيديو من المكتبة"),
...
GITHUB_REPO  = os.environ.get('GITHUB_REPO', '')   # مثال: username/fb-publisher
GITHUB_BRANCH = os.environ.get('GITHUB_BRANCH', 'main')
_gh_cache = {}  # cache في الذاكرة لتقليل API calls

def _gh_filename(filepath):
    """تحويل مسار الملف لاسم في GitHub"""
...
def _gh_put(filename, data, sha=None):
    ...
    try:
        ...
        logger.error(f'GitHub put error: {e}')
    return False

def load_json(filepath, default):
    # لو GitHub مش متاح — استخدم الملف المحلي
    ...
    fname = _gh_filename(filepath)
    if fname in _gh_cache:
        return _gh_cache[fname]['data']
    # حمّل من GitHub
    data, sha = _gh_get(fname)
    if data is not None:
        _gh_cache[fname] = {'data': data, 'sha': sha}
        return data
...
def save_json(filepath, data):
    ...
    # لو GitHub متاح — احفظ فيه كمان
    if not GITHUB_TOKEN or not GITHUB_REPO:
        return
    fname = _gh_filename(filepath)
    sha = _gh_cache.get(fname, {}).get('sha')
    success = _gh_put(fname, data, sha)
    if success:
        # حدّث الـ cache
        _, new_sha = _gh_get(fname)
        _gh_cache[fname] = {'data': data, 'sha': new_sha or sha}
        logger.info(f'Saved to GitHub: {fname}')
...
def ping():
    if not scheduler.running:
        scheduler.start()
        setup_scheduler()
        logger.info("Scheduler restarted via /ping")
    return jsonify({'status': 'alive', 'scheduler': scheduler.running})
