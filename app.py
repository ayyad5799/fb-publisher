...
def generate_content_with_gemini(topic, post_type, page_name):
    import random
    ...
    try:
        ...
        angles = [
            'من زاوية إيجابية ومشجعة',
            'بأسلوب قصصي وشيق',
            'بطريقة مباشرة وعملية',
            'بأسلوب يثير الفضول والتساؤل',
            'من منظور خبير متخصص',
            'بطريقة مرحة وخفيفة الظل',
            'مع أرقام ومعلومات مثيرة للدهشة',
            'بأسلوب التحدي والإلهام'
        ]
        prompt = type_prompts.get(post_type, f'اكتب بوست مناسب عن: {topic}')
        angle = random.choice(angles)
        seed = datetime.now().strftime('%Y%m%d%H%M%S')
        full_prompt = f"""أنت مدير صفحة فيسبوك عربية محترف ومبدع.
{prompt}
الأسلوب المطلوب: {angle}
رقم التنويع: {seed}
متطلبات: باللغة العربية، جذاب ومختلف عن أي منشور سابق، 100-300 كلمة، إيموجي مناسبة، دعوة للتفاعل في النهاية.
اكتب المنشور مباشرة بدون أي مقدمة أو عنوان."""
...
def publish_scheduled_post(page_id_str, schedule_id):
    ...
    content = generate_content_with_gemini(topic, post_type, page['name'])
...
def get_settings():
    ...
    safe['has_api_key'] = bool(settings.get('gemini_api_key'))
    return jsonify(safe)
...
def publish_page(page_id):
    ...
    content = generate_content_with_gemini(page.get('topic', 'عام'), post_type, page['name'])
    success, result = post_to_facebook(page['page_id'], page['access_token'], content)
    type_name = POST_TYPES.get(post_type, post_type)
...
def test_token():
    data = request.json
    try:
        # Try posting endpoint directly to verify token works for publishing
