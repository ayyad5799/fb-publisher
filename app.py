...
def generate_content(topic, post_type, page_name):
    ...
    try:
        ...
        full_prompt = f"""أنت كاتب محتوى عربي محترف ومبدع لصفحة فيسبوك.
{prompt_text}
الأسلوب: {angle}
كود التنويع: {seed}
المتطلبات:
- باللغة العربية الفصيحة أو العامية المناسبة
- فريد ومختلف تماماً عن أي منشور سابق
- من 150 إلى 350 كلمة
- استخدم إيموجي مناسبة بشكل طبيعي
- اختم بدعوة واضحة للتفاعل (لايك أو تعليق أو مشاركة)
- لا تكتب عنواناً أو مقدمة - ابدأ مباشرة بالمحتوى"""
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={api_key}"
        r = requests.post(url, json={"contents": [{"parts": [{"text": full_prompt}]}]}, timeout=30)
        data = r.json()
        logger.info(f"Gemini status: {r.status_code}, keys: {list(data.keys())}")
        if 'candidates' in data and data['candidates']:
            return data['candidates'][0]['content']['parts'][0]['text']
        elif 'error' in data:
            logger.error(f"Gemini API error: {data['error'].get('message','unknown')}")
            return generate_fallback_content(topic, post_type)
        else:
            logger.error(f"Unexpected Gemini response: {str(data)[:200]}")
            return generate_fallback_content(topic, post_type)
