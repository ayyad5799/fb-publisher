...
def generate_content(topic, post_type, page_name, page_id='', personality='mixed'):
    ...
    if post_type not in fixed_types:
        ...
        # صباح؟ اذكار
        elif now_tz.hour in [5,6,7] and random.random() < 0.4:
            post_type = 'adhkar_morning'
        # مساء؟ اذكار مساء
        elif now_tz.hour in [20,21,22] and random.random() < 0.3:
            post_type = 'adhkar_evening'
        # جمعة؟ خطبة
        elif now_tz.weekday() == 4 and now_tz.hour in [10,11,12] and random.random() < 0.5:
